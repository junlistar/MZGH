...
namespace FastReportClient
{
    public partial class SelectReportFrom : Form
    {
        ...
        {
            string sql = $"select report_code,short_name,long_name,report_sql from rt_report_data_fast_net where report_code like '%{txtcode.Text.Trim()}%' order by report_code";//where report_code = 220001
            DataTable dt = DbHelper.ExecuteDataTable(sql);
            dataGridView1.DataSource = dt;
        }
    }
