...
namespace YbjsLib
{
    public partial class YBJSPreview : UIForm
    {
        ...
        private void YBJSPreview_Load(object sender, EventArgs e)
        {
            ...
            //加载下拉框字典数据
            if (!string.IsNullOrEmpty(insutypes))
            {
                var _list = WebApiHelper.DeserializeObject<List<InsutypeVM>>(insutypes);
                jz_insutype.DataSource = _list;
                jz_insutype.ValueMember = "code";
                jz_insutype.DisplayMember = "name";
            }
            if (!string.IsNullOrEmpty(mdtrtCertTypes))
            {
                var _list = WebApiHelper.DeserializeObject<List<MdtrtCertTypeVM>>(mdtrtCertTypes);
                mdtrt_cert_type.DataSource = _list;
                mdtrt_cert_type.ValueMember = "code";
                mdtrt_cert_type.DisplayMember = "name";
            }
            if (!string.IsNullOrEmpty(medTypes))
            {
                var _list = WebApiHelper.DeserializeObject<List<MedTypeVM>>(medTypes);
                med_type.DataSource = _list;
                med_type.ValueMember = "code";
                med_type.DisplayMember = "name";
            }
            if (!string.IsNullOrEmpty(birctrlTypes))
            {
                var _list = WebApiHelper.DeserializeObject<List<BirctrlTypeVM>>(birctrlTypes);
            ...
            if (userInfoResponseModel != null && userInfoResponseModel.baseinfo != null)
            {
                txt_psn_no.Text = userInfoResponseModel.baseinfo.psn_no;
                psn_name.Text = userInfoResponseModel.baseinfo.psn_name;
                gend.Text = userInfoResponseModel.baseinfo.gend == "1" ? "男" : "女";
                brdy.Text = userInfoResponseModel.baseinfo.brdy;


                mdtrt_cert_type.SelectedValue = userInfoResponseModel.baseinfo.psn_cert_type;
                mdtrt_cert_no.Text = userInfoResponseModel.baseinfo.certno;
            }
            if (userInfoResponseModel != null && userInfoResponseModel.insuinfo != null)
            {
                dgv_cbxx.Init();
                dgv_cbxx.AutoGenerateColumns = false;
                dgv_cbxx.DataSource = userInfoResponseModel.insuinfo;

                lbl_insplcadmdvs.Text = userInfoResponseModel.insuinfo[0].insuplc_admdvs;
                jz_insutype.SelectedValue = userInfoResponseModel.insuinfo[0].insutype;
            }

            if (userInfoResponseModel != null && userInfoResponseModel.idetinfo != null)
            {
                foreach (var item in userInfoResponseModel.idetinfo)
                {
