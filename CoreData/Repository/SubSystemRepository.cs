...
namespace Data.Repository
{
    public class SubSystemRepository : RepositoryBase<SubSystem>, ISubSystemRepository
    {
            ...
            try
            {
                ...
                using (IDbConnection connection = DataBaseConfig.GetSqlConnection(DBConnectionEnum.Write))
                {
                    ...
                    try
                    {
                        ...
                        para.Add("@sys_group_code", subSystem.sys_group_code);
                        para.Add("@subsys_id", subSystem.subsys_id);
                        para.Add("@group_no", subSystem.group_no);
        ...
        public MainClientConfig GetMainClientConfig()
        {
            try
            { 
                using (IDbConnection connection = DataBaseConfig.GetSqlConnection(DBConnectionEnum.Write))
                {
                    string sql = GetSqlByTag("mainclientconfig_get");

                    return connection.Query<MainClientConfig>(sql).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
        ...
        public bool UpdateMainClientConfig(string jsonStr)
        {
            try
            {
                MainClientConfig model = JsonConvert.DeserializeObject<MainClientConfig>(jsonStr);
                ...
                using (IDbConnection connection = DataBaseConfig.GetSqlConnection(DBConnectionEnum.Write))
                {
                    string sql = GetSqlByTag("mainclientconfig_update");
